...
export const signup = async (req,res)=>{
    ...
    //hash passwords
        if(!(firstName && email && password)){
            return res.status(400).json({message:"Please fill the required fields and try again"})
        }
...
export const updateProfile = async (req,res)=>{
    try{
        ...
        res.status(500).json({message:"uploading image failed due to server error! try again later"});
    }
}
